...
import Classifier.MyNaiveBayes;
...
public class DownLoadFile {
	...
	private void MakeDirAndSave(String content, String url, String city,
		...
		try {
			String dirPath;
			if (type.length() == 0)
				dirPath = "doc/创客网页/" + city;// 网页存放目录
			else
				dirPath = "doc/创客网页/" + type + "/" + city;// 网页存放目录

			File file = new File(dirPath);
			if (!file.exists()) {
				System.out.println(file.mkdirs());
				File mapfile = new File(dirPath + "/map.txt");
				if (!mapfile.exists()) {
			...
			BufferedWriter fwmap = new BufferedWriter(new FileWriter(dirPath
			        + "/map.txt", true));
			fw.write(content);
			fw.flush();
			fw.close();
			fwmap.write(url + " " + fileName);
			fwmap.newLine();
	...
	public static void main(String[] args) throws Exception {
		...
		System.out
		.println(dlf.downloadFile("http://1010.njnu.edu.cn/", "news"));
	}
}
