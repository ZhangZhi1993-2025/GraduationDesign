...
public class ExtractIncubators extends InfoExtract {
    ...
    public List<Extractable> extractInformation(String html) {
        ...
        while (html.charAt(index) != ' ')
            index++;
        String city = html.substring(0, index);
        html = html.substring(index + 1, html.length() - 1);
    ...
    public boolean Extract(Element root, Incubator info, String city) {
         ...
         * 抽取地点信息
         */
        esuc = ExtractAddress(root, info, city);
        ...
        for (ValueFeature i : PatternStore.valuepattern) {
            ...
            if (!MatchRootList.isEmpty()) {
                ...
                for (int index = 0; index < MatchRootList.size(); index++) {
                    ...
                    if (vflag)// extract successfully,break the whole
                        // loop;
                        break;
                }
            } else
                continue;// if no matched tag, find next pattern.
            if (vflag)// extract successfully, break the loop
                break;
        }
        for (Feature i : PatternStore.allpattern) {
            if (vflag)
                ...
                if (!MatchedParentRoots.isEmpty()) { // if get matched
                    // parent roots'
                    // list.
                    ...
                    for (int index_parent = 0; index_parent < MatchedParentRoots.size(); index_parent++) {
                        ...
                        if (!MatchedKeyRoots.isEmpty()) {
                            ...
                            continue;
                        if (kflag) {// if it has find key, then look for
                            // matched value
    ...
    public boolean IspatternKey(Element root, Feature feature) {
        String pre_tag = "";
        String next_tag = "";
    ...
    public boolean IspatternValue(Element root, ValueFeature feature) {
        String path = PatternStore.CreatePath(root);
    ...
    public boolean IsKey_Value(Element root, Feature feature) {
        String pre_tag = "";
        String next_tag = "";
        String path = PatternStore.CreatePath(root);
    ...
    public boolean ExtractAddress(Element root, Incubator info, String city) {
